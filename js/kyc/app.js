...
function validateStep3() {
  ...
  if (!state.selfie) {
    isValid = false;
    showError('selfie', 'Загрузите фото лица.');
  }

  if (!state.selfieWithDocument) {
    isValid = false;
    showError('selfieWithDocument', 'Загрузите фото с документом.');
  }
...
function updateStatuses() {
  ...
  if (dom.statusSelfie) {
    ...
    dom.statusSelfie.textContent = selfieOk
      ? 'Статус: оба фото загружены и готовы к отправке менеджеру.'
      : 'Статус: загрузите фото лица и фото с документом.';
  }
}
