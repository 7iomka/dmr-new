...
<body class="bg-zinc-50 text-[#1A1D1F] dark:bg-[#0B0E11] dark:text-[#E4E6EB] min-h-screen">
			...
			<main class="flex-1 overflow-y-auto px-4 py-6 lg:p-10">
				<div class="max-w-7xl mx-auto space-y-6">
					<div class="flex flex-col items-start md:flex-row md:items-center justify-between gap-4">
						<div>
              <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-zinc-900 dark:text-white text-left">Финансовый отчёт</h1>
              <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Статистика расходов по категориям</p>
            </div>
            <div class="flex items-center gap-3">
                <!-- Селект Года -->
                <div class="relative">
                    <select class="bg-white dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 text-zinc-800 dark:text-zinc-200 text-sm font-bold py-2.5 pl-4 pr-10 rounded-xl appearance-none focus:outline-none focus:ring-2 focus:ring-accent/50 transition-all cursor-pointer">
                        <option>2025</option>
                        <option>2024</option>
                    </select>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" class="lucide lucide-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-zinc-400 dark:text-zinc-500 w-4 h-4 pointer-events-none">
          ...
          <div class="flex flex-col lg:flex-row gap-6 xl:gap-10 items-start">
            ...
            <div class="flex-1 w-full order-1 md:order-0">
              ...
              <div class="mt-8 p-5 bg-accent/10 border border-accent/20 rounded-2xl max-w-md mx-auto">
                ...
                <p class="text-sm text-zinc-600 dark:text-zinc-400  mb-4">Заинтересованным партнерам отчёты доступны с указанием точных сумм в долларах.</p>
                <button class="px-4 py-3 bg-accent hover:bg-accent/90 text-white text-sm font-bold rounded-lg transition-all w-full">
                  Подать заявку
                </button>
              </div>
            </div>
            ...
            <div class="w-full lg:w-auto xl:w-[320px] lg:sticky lg:top-0 flex flex-col items-center">
              <div class="relative w-[240px] h-[240px] md:w-[280px] md:h-[280px]">
                <svg id="main-svg" viewBox="0 0 100 100" class="transform -rotate-90 w-full h-full">
                  <circle cx="50" cy="50" r="40" fill="transparent" stroke="currentColor" class="text-zinc-200 dark:text-zinc-800" stroke-width="8" />
  ...
  <script>
      (function () {
        const data = [
            { label: "Выплата по партнерской программе", value: 31.45, color: "#EAB308" },
            { label: "Патентование", value: 26.05, color: "#6366F1" },
            { label: "Аренда платформы", value: 11.87, color: "#10B981" },
            { label: "Заработная плата сотрудников", value: 11.25, color: "#0EA5E9" },
            { label: "Мероприятие (Россия)", value: 5.16, color: "#D946EF" },
            { label: "Аренда офисов", value: 4.89, color: "#F97316" },
            { label: "Транспортные расходы", value: 2.62, color: "#64748B" },
            { label: "Оплата партнёров компаний инженеров", value: 2.08, color: "#F43F5E" },
            { label: "Оплата кредита транспортного средства", value: 1.77, color: "#8B5CF6" },
            { label: "Юридические услуги", value: 1.12, color: "#06B6D4" },
            { label: "Расходы на транспортные средства", value: 0.77, color: "#84CC16" },
            { label: "Организация клипов", value: 0.66, color: "#EC4899" },
            { label: "Товарный знак (Китай)", value: 0.31, color: "#F59E0B" }
        ];
        ...
        data.forEach((item, index) => {
            ...
            div.className = "category-item flex items-center justify-between card-simple py-2 transition-all hover:border-accent/30 cursor-pointer group fade-in";
